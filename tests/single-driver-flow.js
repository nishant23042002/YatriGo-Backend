...
runScenario("single-driver-flow", async () => {
  ...
  try {
    ...
    await runStep("driver goes online", () =>
      emitWithAck(driverSocket, "go_online", {
    ...
    const newRideRequestPromise = waitForEvent(
      driverSocket,
      "new_ride_request",
    );
    const rideRequestedResponse = await runStep("customer requests ride", () =>
      emitWithAck(customerSocket, "request_ride", {
        eventId: createId("evt"),
    ...
    await runStep("driver accepts ride", () =>
      emitWithAck(driverSocket, "accept_ride", {
        eventId: createId("evt"),
        rideId,
      }),
    );
    ...
    const locationUpdatePromise = waitForEvent(
      customerSocket,
    ...
    await runStep("driver marks arriving", () =>
      emitWithAck(driverSocket, "ride_arriving", {
        eventId: createId("evt"),
        rideId,
      }),
    );
    ...
    await runStep("driver starts ride", () =>
      emitWithAck(driverSocket, "ride_started", {
        eventId: createId("evt"),
        rideId,
      }),
    );
    ...
    await runStep("driver completes ride", () =>
      emitWithAck(driverSocket, "ride_completed", {
        eventId: createId("evt"),
        rideId,
      }),
    );
    const completedRide = await waitForRideStatus(rideId, "COMPLETED");
      ...
      completedRide.driverId === driverId,
      "Completed ride has wrong driver",
    );

    const customerActiveRide = await redis.get(
      `customer:${customerId}:activeRide`,
    );
    ensure(
      !customerActiveRide,
      "Customer active ride pointer still exists after completion",
    );

    const driverActiveRide = await redis.get(`driver:${driverId}:activeRide`);
    ensure(
      !driverActiveRide,
      "Driver active ride pointer still exists after completion",
    );

    const fetchedRide = await fetchRide(rideId);
    ensure(
      fetchedRide.status === "COMPLETED",
      "Ride fetch did not return completed state",
    );
